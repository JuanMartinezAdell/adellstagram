...
<body class="bg-gray-100">
    <header class="p-5 border-b bg-white shadow">
        <div class="container mx-auto flex justify-between items-center">
            <h1 class="text-3xl font-black">AdellStagram</h1>
            ...
            @auth
                <nav class="flex gap-2 items-center">
                    ...
                    <a class="font-bold uppercase text-gray-600 text-sm"
                        href="{{ route('posts.index', auth()->user()->username) }}">Hola:</a> <span
                        class="font-normal">{{ auth()->user()->username }}</span>

                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="font-bold uppercase text-gray-600 text-sm"
                            href="{{ route('logout') }}">Cerrrar
                            Sesión</button>
                    </form>
                </nav>
            @endauth
    ...
    <footer class="mt-10 text-center p-5 text-gray-500 font-bold uppercase">
        AdellStagram - Todos los derechos reservados {{ now()->year }}
    </footer>
</body>
